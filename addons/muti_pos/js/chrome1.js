...
/* ------- The Clerk Name Widget ------- */

// Displays the current cashier's name and allows
// to switch between cashiers.

var ClerknameWidget = PosBaseWidget.extend({
    template: 'ClerknameWidget',
    init: function(parent, options){
        options = options || {};
        this._super(parent,options);
    },
    renderElement: function(){
        ...
        this.$el.click(function(){
            self.click_clerkname();
        });
    },
    click_clerkname: function(){
        var self = this;
        this.gui.select_user({
            'security':     true,
            'current_user': this.pos.get_clerk(),
            'title':      _t('Change Clerk'),
        }).then(function(user){
            self.pos.set_clerk(user);
            self.renderElement();
        });
    },
    get_name1: function(){
        var user = this.pos.clerk;
        if(user){
            return user.nickname;
        }else{
            return "Unassigned";
...
/* ------- The Checker Name Widget ------- */

// Displays the current cashier's name and allows
// to switch between cashiers.

var CheckernameWidget = PosBaseWidget.extend({
    template: 'CheckernameWidget',
    init: function(parent, options){
        options = options || {};
        this._super(parent,options);
    },
    renderElement: function(){
        ...
        this.$el.click(function(){
            self.click_checkername();
        });
    },
    click_checkername: function(){
        var self = this;
        this.gui.select_user({
            'security':     true,
            'current_user': this.pos.get_checker(),
            'title':      _t('Change Checker'),
        }).then(function(user){
            self.pos.set_checker(user);
            self.renderElement();
        });
    },
    get_name2: function(){
        var user = this.pos.checker;
        if(user){
            return user.nickname;
        }else{
            return "Unassigned";
    ...
    widgets: [
        {
            ...
            'name':   'clerkname',
            'widget': ClerknameWidget,
            'replace':  '.placeholder-ClerknameWidget',
        },{
            'name':   'checkername',
            'widget': CheckernameWidget,
            'replace':  '.placeholder-CheckernameWidget',
        },{
            'name':  'keyboard',
...
return {
    ...
    ClerknameWidget: ClerknameWidget,
    CheckernameWidget: CheckernameWidget,
};
});
